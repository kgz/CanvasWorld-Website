...
#
# Usage:
#   ./bin/od-import-project.sh          # once, or after resetting OD
#   ./bin/od-design-run.sh --issue 12 "Design canvasworld-home.html ..."
#   ./bin/od-design-run.sh --issue 12 --new-conversation "#12 Home polish" "Design ..."
#   ./bin/od-design-run.sh -f design/canvasworld-prototype/od-design-prompt.md
#
# Env (optional):
#   OD_PROJECT          default: from design/canvasworld-prototype/.od-project.json
#   OD_CONVERSATION     default: from manifest
#   OD_DAEMON_URL       default: http://127.0.0.1:7456
#   OD_WEB_URL          default: http://127.0.0.1:7456
#   OD_AGENT_ID         default: cursor-agent
#   OD_MODEL            default: auto
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
MANIFEST="$ROOT/design/canvasworld-prototype/.od-project.json"
DAEMON="${OD_DAEMON_URL:-http://127.0.0.1:7456}"
WEB="${OD_WEB_URL:-http://127.0.0.1:7456}"
AGENT="${OD_AGENT_ID:-cursor-agent}"
MODEL="${OD_MODEL:-auto}"

if [[ -z "${OD_PROJECT:-}" || -z "${OD_CONVERSATION:-}" ]]; then
	if [[ ! -f "$MANIFEST" ]]; then
		echo "No OD project manifest at $MANIFEST" >&2
		echo "Run: ./bin/od-import-project.sh" >&2
		exit 1
	fi
	OD_PROJECT="${OD_PROJECT:-$(python3 -c 'import json; print(json.load(open("'"$MANIFEST"'"))["projectId"])')}"
	OD_CONVERSATION="${OD_CONVERSATION:-$(python3 -c 'import json; print(json.load(open("'"$MANIFEST"'"))["conversationId"])')}"
fi

PROJECT="$OD_PROJECT"
CONVERSATION="$OD_CONVERSATION"
...
run = api("POST", "/api/runs", run_body)
